...
apps.get("/",async function(req,res){
 ...
 apps.post("/signup",async function(req,res){
    ...
    var userexist=await clients.db("spotify").collection("user").findOne({name:name})
    console.log(data)
    if(!userexist){res.send(await clients.db("spotify").collection("user").insertOne(data))}
    else{res.status(404).send({msg:"already exist"})}
